...
it.todo("is the solution to day 5 challenge 2", async () => {
  const input = await readFile(join(__dirname, "input.txt"), "utf-8");
  const map = parseInput(input);

  let min = -1;
  for (let i = 0; i < map.seeds.length; i += 2) {
    for (let j = map.seeds[i]; j < map.seeds[i] + map.seeds[i + 1]; j++) {
      const location = processSeed(j, map.chain);
  ...
  console.log("Day 5 Challenge 2:", min);
});
